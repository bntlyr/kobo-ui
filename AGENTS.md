...
Kobo UI provides several built-in color themes (e.g., Zinc, Slate, Blue, Rose, Orange).
- These preset themes are automatically generated by the CLI into `src/styles/themes.css`.
- To apply or change a theme, simply apply the corresponding class to the `<body>` or `<html>` tag (e.g., `<body class="theme-blue">`). Do NOT manually redefine HSL variables to change themes; simply use the classes provided in `themes.css`.

### 2.4 Toast Container Setup
...
7. **Icons**: Use `@lucide/angular` icons rather than emoji or custom SVGs. You must import the specific icon (e.g., `LucideUser`) and render it via the directive `lucideUser` or `<lucide-icon [img]="LucideUser" />`.
...
9. **Reactive Forms**: Form controls (`k-checkbox`, `k-switch`, `k-radio-group`, `k-slider`, `k-select`) implement `ControlValueAccessor`. Always prefer using `[formControl]="control"` or `formControlName="name"` (via `ReactiveFormsModule`) instead of `[(ngModel)]` for complex forms. Use `control.invalid && control.touched` to conditionally render `<k-form-error>`.
...
<k-form-field>
  ...
  <input k-input id="username" [formControl]="usernameControl" />
    ...
    <k-sidebar>
      ...
      <k-sidebar-content>
        <k-sidebar-group>
          ...
          <k-sidebar-menu>
            <k-sidebar-menu-item>
              <button kSidebarMenuButton [isActive]="true">Dashboard</button>
            </k-sidebar-menu-item>
            <k-sidebar-menu-item>
              <button kSidebarMenuButton>Analytics</button>
            </k-sidebar-menu-item>
          </k-sidebar-menu>
        </k-sidebar-group>
      </k-sidebar-content>
...
| `class="dark:bg-slate-900 dark:text-white"` | `class="bg-background text-foreground"` (CSS variables handle dark mode natively!) |
| `<button k-sidebar-menu-button>` | `<button kSidebarMenuButton>` (camelCase directive) |
...
| `<lucide-icon name="user" />` | `<lucide-icon name="user" aria-hidden="true" />` or `aria-label="User"` if actionable |
| `class="mt-4 mb-4"` (manual margins) | `class="space-y-4"` (Tailwind spacing utilities on parent) |
